...
In \secref{sec:gamification-sweng} we discuss the applicability of gamification principles in a software engineering context, while in \secref{sec:gamification-framework} we introduce a framework for applying gamification to a software project.
...
\item An illustration, through several concrete examples and scenarios, of how our gamification framework can be used for the gamification of diverse software engineering activities (\secref{sec:gamification-example-bugs} and \secref{sec:gamification-example-gems}).
...
First, there is the question of \emph{privacy}, as gamified systems and contexts can be misused to collect a vast amount of information about the players.
...
We need an approach which supports what McGonigal~\cite{McGo2011a} identified as the 4 key aspects of gamification: \emph{Satisfying work}, the \emph{experience/hope of being successful}, a \emph{social connection}, and a deeper \emph{meaning}.
...
Our framework is based on the concept of \emph{Activity} (depicted in \figref{fig-frame}), which is composed of \emph{Analysis}, \emph{Implementation}, and \emph{Testing}.
Each activity pertains to a specific user type (role), present in gamification systems, which can be \begin{inparaenum}[i)] \emph{\item Observer}, who acts in read-only mode and does not contribute anything new, \emph{\item Writer}, who only interacts by modifying existing contents and \emph{\item Solver}, who accomplishes the objectives of the gamification system\end{inparaenum}.
People interacting with a gamification system dynamically switch between these roles.

An \textbf{Activity} consists of an \emph{ID} formed by the initial letter of the role plus an incremental number (\eg the first activity listed in Writer has the ID ``W1''), a brief \emph{description}, and a list of pertinent gamification building blocks (which we describe later).
Each activity is structured in the following way:

\begin{enumerate}

\item{\textbf{Analysis:}} Each activity within the gamification environment must come with an easily understandable \emph{rationale} to connect to the global objectives of the environment, and the \emph{emotional goal} we want to achieve in the people.
Without this analysis step, a gamification effort risks turning into a random set of arbitrary decisions.

\item{\textbf{Implementation:}} To implement an activity the \emph{actors} must be known and we need to understand which gamification \emph{dynamics} they will be involved in, which represent the tactics to engage people in a specific activity.
This is instantiated with game components we call \emph{meta}, following Taje's nomenclature.
Last, one must ponder the \emph{hazards} that can arise from a game structure (algorithmic issues, misbehavior, hardware requirements, \etc).

\item{\textbf{Testing:}} The last component is devoted to testing the activities, where it must be understood which entities are the \emph{target} of the testing, which \emph{methodology} can be used to perform the testing, and lastly, which the \emph{expected results} and the {actual results}, to facilitate an iterative approach to the development of a gamification environment.
...
Before we can provide concrete examples of how the framework is to be used, we need one last missing and fundamental piece: Each activity hinges on one or more \textbf{building blocks}, which also denote the particular categories of gamification it belongs to.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Gamification Building Blocks}\label{sec:gamification-building-blocks}

The ten building blocks we present here have been identified during the construction of several software engineering gamification environments we have constructed, and which we briefly present in a later section.
...
\buildingblock{Production}{production}
{After registering, users must become immediately productive in the environment, because delays in starting using the platform may result in a drop of interest and cause users to quit.
...
\emph{Example:} Bob provides useful comments to a bug being handled by someone else.
...
\emph{Example:} Bob provides his first bug fix.
  \item \textbf{Hive}: proposing an idea to improve the platform and community life.
\emph{Example:} Bob proposes to introduce a ``Bug of the day'' notification mechanism.
\end{itemize}}

\buildingblock{Bravery}{bravery}
{In the production process, users may attempt hard tasks.
...
\emph{Example:} Bob fixes an old bug that made many people despair and is awarded by the community with an ``Unstoppable'' badge.}

\buildingblock{Scrum}{scrum}
{In Rugby, Scrum is a way of restarting the game: Players bind together in order to make the other team collapse and take possession of the ball.
...
\emph{Example:} Bob spends time assigning bug reports to users that he knows to be expert in the area, or tagging easy bugs for newbies.}

\buildingblock{Chameleon}{chameleon}
...
\emph{Example:} Bob closes five bug reports with a single fix.
The system administrators create a special ``Epic'' badge, and add it to the possible badges users can achieve.}

\buildingblock{Thunderbolt}{thunderbolt}
{When users become experts, with many obtained rewards, they might fall into a state of boredom.
...
\emph{Example:} Bob has not participated in any bug fixing activity for the last month.
He and similar users are notified about a complex bug and a bounty for fixing that bug.}

\buildingblock{Phasing}{phasing}
{Users may perform actions in the virtual world that, in reality, produce a permanent impact on the surrounding.
...
\emph{Example:} Bob tags bugs that are old and inactive, but still interesting.
The administrator then creates a new section highlighting such bugs, and acknowledges the contribution of Bob.}

\buildingblock{Beautification}{beautification}
{Appearance, even if only virtual, is important to many.
...
\emph{Example:} As Bob becomes an expert bug fixer, his avatar (for example depicted as a warrior) is decorated with better clothes and weapons.
After a period of inactivity due to his (real) holidays, Bob's avatar is depicted out of shape and with a broken sword.}

\buildingblock{Champagne}{champagne}
{Since achievements inside the magic world are important to users, they want to celebrate their success not only within the virtual world, but also in the real one.
\emph{Example:} Bob is looking for a new job and on his curriculum puts a link to his profile in the bug tracker, as proof of his expertise\footnote{Mozilla is already proposing a similar concept at http://openbadges.org/}.}

\buildingblock{Ascension}{holyascension}
{A game usually has an end.
It is intrinsically rewarding and fulfilling to see the words \emph{The end} on a screen, even though the actual satisfaction comes by what was done along the way.
...
\emph{Example:} Bob has been a productive bug hunter for many years, and is rewarded by the environment admins by being invited to become also an admin.}

\paragraph{Putting everything together.} In the following we provide two concrete examples of gamification environments we have been developing.
...
\badge{heracles_badge} \textbf{Heracles}\footnote{Heracles was the greatest hero in Greek mythology.
...
In the context of a code review tool that we were building in the research group, we designed a gamification environment named \textit{The Empire of Gemstones}, to exploit the parallelism between collecting gemstones and improving the quality of the code.
