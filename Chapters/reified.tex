...
In \chref{ch:stacktraces} we saw that  collecting stack traces can teach us stories about a system that we hardly would have noted otherwise.
...
Writing code is only a small part of the process: Several phases such as design, testing, and maintenance, play a fundamental role in the success of a project.
...
% that is implicitly more complicated.
...
To overcome the difficulties of dealing with plain text to analyze programs, researchers tried to employ different models to represent source code, like \textsc{srcML}~\cite{Male2002a} or \textsc{JavaML}~\cite{Badr2000}.
...
The cognitive cost for building a mental model of a system takes a significant toll on the energy of developers.
...
We now present the implementation details of the framework for the Smalltalk programming language, while in \secref{sec:reified-summary} we will also discuss the challenges of generalizing such an approach to other programming languages.
...
The two applications proposed by default by our approach consist of a local data browser, and a customized reporter.
...
\figref{fig:announcer-report} shows a bug report submitted to the \pha issue tracker, describing a problem with a tool where selecting an item from a menu would trigger the opening of two duplicate windows, instead of one.
...
To implement the collector we need to define the strategies for its activation and to describe the data extraction.
...
This data can then be serialized using a framework like \textit{STON},\seeurl{https://github.com/svenvc/ston} so that a developer can still access the parts of the objects involved with the exception.
...
By serializing and sending this information, we can collect and group this data from all the failed tests to generate a report containing all the objects in a wrong state, and navigate it to identify the cause for the failing tests.
...
There is also an interesting aspect to consider pertaining to how we used a collector in \secref{sec:reified-story-testing} to save and submit the whole \pha image.
