...
\secref{sec:related} analyzes related work on debugging data.
...
In \secref{sec:stacktraces-dataset} we detail the Pharo system and the approach we used to collect the stack trace data.
...
\begin{center}
  ...
  \caption{Distribution of the stack traces on the Pharo system using a city like visualization, where each building is a class.
Pharo is composed of 14,045 classes distributed among 557 packages.
...
\figref{fig:stackcity-classes} shows the impact that runtime exceptions have on the Pharo system, adopting a city visualization~\cite{Wett2011a}.
...
In Pharo, everything is modeled as an object.
...
We visualized the data to highlight the parts of the Pharo system that were involved in the collected exceptions: \figref{fig:stackcity-methods} shows a city visualization of the stack trace data mapped on the whole Pharo system.
...
We mined the Pharo bug tracker to collect the bug reports produced during the development of the platform.
...
By continuing to read the report, we can see that the problem has been further investigated, and that a \emph{slice} (a piece of submitted code, that in Pharo works in a similar way of a patch) has been proposed and is being tested by the continuous integration server of the project.
...
We presented \shr, a tool that seamlessly integrates into the Pharo system to collect stack traces produced during the arise of runtime executions in the system.
