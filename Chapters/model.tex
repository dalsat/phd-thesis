...
%\AB{Maybe it could be expanded a bit? I have the impression that there are more results that are interesting to report here}
 providing more fields in a report relates to the fixing time: In particular, the bug reports with longer descriptions tend to be solved quicker.
...
\textbf{Conclusion.} \figref{fig:survey} shows that some elements are perceived as more difficult to provide when submitting a bug report.
...
{\bfseries Anatomy of a Report.} Issue trackers are platform independent: They share a flexible common core structure to meet all the possible requirements of a software system's development process.
...
{\bfseries The Model.} From the list of entities in a tracker and their list of metadata, we built a model to access the data.
...
\item {\it Edit:} A change in the existing report.
...
\item {\it Link:} The relation (if any) to another report.
...
\item {\it Unused states:} Some states are rarely used: \textsc{awaiting feedback} and \textsc{ready to commit}.
...
{\bfseries Preparing the data.} To interact with the dataset, we created a \emph{vector space model} to allow us to test statistical and machine learning approaches.
...
{\bfseries Predicting the Lifetime of a Defect.} We studied the core fields that are the most relevant in impacting the lifetime.
...
{\bfseries Threats to Validity.} Dealing with large amounts of data can pose some problems in creating an abstraction sufficiently broad to comprise all the aspects of the data, but still specific enough to capture its details.
...
{\bfseries Future Work.} Developers tend to prefer simpler models to depict software defects.
