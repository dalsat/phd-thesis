...
Past research found that users do not provide the information that developers deem \emph{ideally useful} to fix a bug~\cite{Zimm2010a}.
...
Our results show that providing more specific fields in a report relates to the fixing time: In particular, the bug reports with longer descriptions tend to be solved quicker.
...
According to Zimmermann \etal the information provided by submitters of bug reports can be partial or incorrect~\cite{Zimm2010a}.
...
In fact, we are especially interested in knowing the point of view of submitters of bug reports, rather than the view of the developers that ``consume'' these reports~\cite{Zimm2010a}.
...
The large volume of data we collected enables us to explore the usage of issue trackers and to investigate the common practices of bug tracking.
...
To answer RQ3, we build a transition diagram of all the state changes for each issue tracker, to highlight the common patterns in the growth of a report, and we weight the nodes and edges of the diagram with the values from the dataset.
...
To introduce such a degree of tolerance, we divide the reports into \emph{buckets} according to their lifetime.
 ...
 Using bucketing we can deal with discrete values and adopt a classification approach, as opposed to a regression to predict a continuous variable.
...
From our study using PCA, we observe that there exists a set of core elements of a bug report that impact and influence its lifetime.
...
By the usage that we observed in our dataset, we gather that developers tend to use simple models in describing software defects.
